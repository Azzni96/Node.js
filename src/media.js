const mediaItems = [
    {
      "media_id": 9632,
      "filename": "ffd8.jpg",
      "filesize": 887574,
      "title": "Favorite drink",
      "description": "",
      "user_id": 1606,
      "media_type": "image/jpeg",
      "created_at": "2023-10-16T19:00:09.000Z"
    },
    {
      "media_id": 9626,
      "filename": "dbbd.jpg",
      "filesize": 60703,
      "title": "Miika",
      "description": "My Photo",
      "user_id": 3671,
      "media_type": "image/jpeg",
      "created_at": "2023-10-13T12:14:26.000Z"
    },
    {
      "media_id": 9625,
      "filename": "2f9b.jpg",
      "filesize": 30635,
      "title": "Aksux",
      "description": "friends",
      "user_id": 260,
      "media_type": "image/jpeg",
      "created_at": "2023-10-12T20:03:08.000Z"
    },
    {
      "media_id": 9592,
      "filename": "f504.jpg",
      "filesize": 48975,
      "title": "Desert",
      "description": "",
      "user_id": 3609,
      "media_type": "image/jpeg",
      "created_at": "2023-10-12T06:59:05.000Z"
    },
    {
      "media_id": 9590,
      "filename": "60ac.jpg",
      "filesize": 23829,
      "title": "Basement",
      "description": "Light setup in basement",
      "user_id": 305,
      "media_type": "image/jpeg",
      "created_at": "2023-10-12T06:56:41.000Z"
    }
  ];

const getMediaItems = (res) => { 
    res.json(mediaItems);
    }

const postMediaItem = (req, res) => {
    const newMediaItem = req.body;
    newMediaItem.media_id = mediaItems[mediaItems.length - 1].media_id + 1;
    mediaItems.push(newMediaItem);
    res.status(201).json({
        message: 'Media item added successfully',
        media_id: newMediaItem.media_id
    });
}

const getMediaItemsById = (req, res) => {
    const mediaId = parseInt(req.params.media_id);
    const mediaItem = mediaItems.find(item => item.media_id === mediaId);
    if (mediaItem) {
        res.json(mediaItem);
    } else {
        res.status(404).json({error: 'Media item not found'});
    }
}

export { getMediaItems, postMediaItem, getMediaItemsById, mediaItems};